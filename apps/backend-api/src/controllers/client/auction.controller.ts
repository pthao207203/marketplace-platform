...
export async function placeBid(req: Request, res: Response) {
  try {
    ...
    try {
      const userDoc = await UserModel.findById(String(user.sub)).select("userAddress").lean<any>();
      const hasAddr = userDoc && Array.isArray(userDoc.userAddress) && userDoc.userAddress.length > 0;
      ...
      console.error("placeBid: failed to check user address", e);
      return sendError(res, 400, "Could not verify shipping address. Please try again.");
    }
