...
import { statusValueToName, eventCodeToStatusValue } from '../constants/order.constants';
...
export async function upsertFromProvider(shipmentId: string, tmLike: any) {
  ...
  async function attemptRefund(shId: string) {
    try {
      ...
      if (!order) return;
      if (order.returnRequest && order.returnRequest.status === 'approved' && !order.returnRequest.refundProcessed) {
        const buyerId = String(order.orderBuyerId);
        const refundAmount = Number(order.orderTotalAmount || 0);
        const topup = { amount: refundAmount, currency: 'VND', bank: { bankName: 'REFUND' }, transactionId: `REFUND-${order._id}-${Date.now()}`, status: 'completed', createdAt: new Date() } as any;
        await User.updateOne({ _id: buyerId }, { $inc: { 'userWallet.balance': refundAmount }, $push: { 'userWallet.topups': topup }, $set: { 'userWallet.updatedAt': new Date() } });
        await OrderModel.updateOne({ _id: order._id, 'returnRequest.refundProcessed': { $ne: true } }, { $set: { 'returnRequest.refundProcessed': true, 'returnRequest.refundAmount': refundAmount, 'returnRequest.status': 'completed', orderStatus: 4, orderPaymentStatus: 'refunded' } });
      }
    } catch (e) {
  ...
  try {
    if (sh && typeof sh.currentStatus === 'number' && sh.currentStatus === SHIPMENT_STATUS.RETURNED) {
      if (sh.orderId) {
        const ordCheck: any = await OrderModel.findById(sh.orderId).select('returnRequest').lean();
        if (ordCheck && ordCheck.returnRequest && ordCheck.returnRequest.refundProcessed) {
          console.info('upsertFromProvider: shipment already RETURNED and refundProcessed - skipping', { shipmentId });
          return;
        }
      }
    }
  ...
  console.log('upsertFromProvider', { shipmentId, status, statusName, currentStatusName, shouldPromote });
  if (shouldPromote) {
    ...
    if (!already) {
      ...
      try {
        if (derivedStatus && derivedStatus !== SHIPMENT_STATUS.UNKNOWN) {
          const current = (sh && typeof sh.currentStatus === 'number') ? sh.currentStatus : undefined;
          const derivedName = statusValueToName(derivedStatus);
          const currentName = current != null ? statusValueToName(current) : 'CREATED';
          const promote = ((RANK as any)[derivedName] ?? 0) > ((RANK as any)[currentName] ?? 0);
          if (promote) {
            await ShipmentModel.updateOne({ _id: shipmentId }, { $set: { currentStatus: derivedStatus, lastSyncedAt: new Date() } });
