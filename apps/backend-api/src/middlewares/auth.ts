...
export function requireShopOrAdminAuth(req: Request, res: Response, next: NextFunction) {
  try {
    const auth = req.headers.authorization || (req.body && req.body.token) || req.query.token;
    if (!auth) return res.status(401).json({ success: false, error: { message: 'Missing token' } });
    const token = String(auth).startsWith('Bearer ') ? String(auth).slice(7) : String(auth);
    ...
    if (!payload || typeof payload.userRole === 'undefined') {
      return res.status(403).json({ success: false, error: { message: 'Insufficient privileges' } });
    }
    const roleCode2 = resolveRoleCode(payload.userRole);
    if (Number.isNaN(roleCode2) || (roleCode2 !== USER_ROLE.SHOP && roleCode2 !== USER_ROLE.ADMIN)) {
      return res.status(403).json({ success: false, error: { message: 'SHOP or ADMIN privileges required' } });
    }
    req.user = { ...payload, token };
    return next();
  } catch (err: any) {
