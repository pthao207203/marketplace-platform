...
export function startSimulation(shipmentId: string) {
  ...
  s.events.push({ eventCode: eventCodeNameToValue(ev.code), description: `Simulated ${ev.code}`, eventTime: now, raw: { simulated: true } } as any);
        // map code to numeric status
        let status = s.currentStatus;
        switch (ev.code) {
          case 'PICKED_UP': status = 2; break;
          case 'ARRIVAL_FACILITY': status = 4; break;
          case 'DEPARTURE_FACILITY': status = 3; break;
          case 'OUT_FOR_DELIVERY': status = 5; break;
          case 'DELIVERED': status = 6; break;
          default: status = s.currentStatus;
        }
